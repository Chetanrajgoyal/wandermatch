...
function getNavHTML(activePage = "", darkHero = false) {
  ...
  <!-- Logo -->
  <a href="index.html" class="logo-kibi flex items-center justify-center h-10 px-5 bg-white/90 dark:bg-[#161B22] backdrop-blur-md rounded-full font-heading font-bold text-[1.25rem] tracking-tight text-[#005da7] dark:text-white hover:bg-white dark:hover:bg-[#1f2830] transition-colors shadow-sm border border-white/20 dark:border-white/10">
    Kibi
  </a>
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-4 mb-12">
      ...
      <div class="md:col-span-1 md:justify-self-center">
        <ul class="space-y-3 text-sm">
          ...
          <li><a href="index.html#faq" class="text-white/60 hover:text-white transition-colors">FAQ</a></li>
        </ul>
      </div>
        ...
        <div id="profileDropdown" class="profile-dropdown absolute top-[calc(100%+8px)] right-0 w-[300px] bg-white border border-slate-200/60 shadow-xl shadow-black/8 rounded-xl overflow-hidden p-5 flex flex-col gap-4 z-[100] opacity-0 invisible translate-y-1 transition-all duration-200 pointer-events-none">
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col min-w-0">
              <span class="font-semibold text-[15px] text-slate-900 dark:text-slate-100 leading-tight truncate">${displayName}</span>
              <span class="text-[13px] text-slate-500 dark:text-slate-400 font-normal truncate">${displayEmail}</span>
            </div>
          </div>
          <div class="h-px w-full bg-slate-100 dark:bg-white/10"></div>
          <div class="flex flex-col gap-1">
            <a href="dashboard.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-[#1b3a5f] text-slate-700 dark:text-slate-200 hover:text-[#005da7] dark:hover:text-[#a4c9ff] transition-colors duration-150 group w-full text-left no-underline">
              <svg class="w-[18px] h-[18px] text-slate-400 dark:text-slate-500 group-hover:text-[#005da7] dark:group-hover:text-[#a4c9ff] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
              <span class="text-[14px] font-semibold">Dashboard</span>
            </a>
            <button id="themeToggleBtn" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-slate-100 transition-colors duration-150 group w-full text-left cursor-pointer">
            ...
            <button id="dropdownLogoutBtn" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-red-500 hover:text-red-600 transition-colors duration-150 group w-full text-left cursor-pointer">
              <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 0 013-3h4a3 3 0 013 3v1"/></svg>
...
function confirmModal(message, onConfirm) {
  ...
  backdrop.className = 'fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 transition-opacity duration-200';
  backdrop.innerHTML = `
    <div class="bg-white dark:bg-[#161B22] rounded-2xl shadow-2xl max-w-sm w-[calc(100%-2rem)] p-6 transform scale-95 transition-transform duration-200" id="confirmModalBox">
      <div class="flex items-start gap-4">
        ...
        <div>
          <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Delete trip?</h3>
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">${message}</p>
        </div>
      </div>
      <div class="flex justify-end gap-3 mt-6">
        <button id="confirmModalCancel" class="px-4 py-2 rounded-full text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">Cancel</button>
        <button id="confirmModalConfirm" class="px-4 py-2 rounded-full text-sm font-medium bg-red-500 text-white hover:bg-red-600 transition-colors shadow-sm">Delete</button>
      </div>
    </div>
  `;
  ...
  function close() {
    ...
    backdrop.querySelector('#confirmModalBox').classList.remove('scale-100');
  ...
  requestAnimationFrame(() => {
    ...
    backdrop.querySelector('#confirmModalBox').classList.add('scale-100');
  });

  backdrop.querySelector('#confirmModalCancel').addEventListener('click', close);
  backdrop.querySelector('#confirmModalConfirm').addEventListener('click', () => {
    close();
